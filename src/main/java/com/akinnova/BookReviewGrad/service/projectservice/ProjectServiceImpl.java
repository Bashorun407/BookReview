//package com.akinnova.BookReviewGrad.service.projectservice;
//
...
//                .filter(x-> x.getProjectCompletionStatus() == PENDING)
...
//        List<Project> projectList = projectRepository.findAll().stream()
//                .filter(Project::getActiveStatus)
//                .filter(x-> x.getProjectCompletionStatus() == STARTED)
//                .toList();
//
//        if(projectList.isEmpty())
...
//        List<Project> projectList = projectRepository.findAll().stream()
//                .filter(Project::getActiveStatus)
//                .filter(x-> x.getProjectCompletionStatus() == COMPLETED)
//                .sorted(Comparator.comparing(Project::getTitle))
//                .toList();
//
//        if(projectList.isEmpty())
...
//        project.setProjectCompletionStatus(serviceProviderUpdateDto.getProjectCompletionStatus());
...
//        projectRepository.save(project);
//
//        logger.info("Project has been updated by service provider");
//
//        // TODO: 05/09/2023 Email should be sent to the project owner that a service provider has accepted the project
//
//        //Getting the details of the user
...
//                        projectId)));
//
//        project.setProjectLevelCompletionApproval(updateDto.getProjectLevelCompletionApproval());
//
//        projectRepository.save(project);
...
////                .filter(Project::getActiveStatus)
////                .skip(pageNum).limit(pageSize).map(ProjectResponseDto::new).collect(Collectors.toList())));
////
////    }
//
//}
